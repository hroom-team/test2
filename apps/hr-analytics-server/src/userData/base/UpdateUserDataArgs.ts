/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { UserDataWhereUniqueInput } from "./UserDataWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { UserDataUpdateInput } from "./UserDataUpdateInput";

@ArgsType()
class UpdateUserDataArgs {
  @ApiProperty({
    required: true,
    type: () => UserDataWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => UserDataWhereUniqueInput)
  @Field(() => UserDataWhereUniqueInput, { nullable: false })
  where!: UserDataWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => UserDataUpdateInput,
  })
  @ValidateNested()
  @Type(() => UserDataUpdateInput)
  @Field(() => UserDataUpdateInput, { nullable: false })
  data!: UserDataUpdateInput;
}

export { UpdateUserDataArgs as UpdateUserDataArgs };
